...
class AssetParser(object):
    ...
    def __init__(self, get_alls=None, get_stdt=None):
        self._grammar = json.loads(
            pkgutil.get_data(__name__, "data/asset_parser.json").decode()
        )
        self._alls = get_alls
        self._stdt = get_stdt
    ...
    def __formatted_version(self, raw_version=""):
        ...
        return (
            (version[0] or "0") + "." + (version[1] or "0") + "." + (version[2] or "0")
        )
    ...
    def __evaluate(self, statement):
        ...
        if operator == "in":
            try:
                return (
                    True
                    if int(optional_value) in map(int, statement["value"])
                    else False
                )
            except:
                return False
        ...
        if operator == "nin":
            try:
                return (
                    True
                    if int(optional_value) not in map(int, statement["value"])
                    else False
                )
            ...
            try:
                return (
                    ...
                    if semver.VersionInfo.parse(
                        self.__formatted_version(optional_value)
                    ).compare(self.__formatted_version(statement["value"]))
                    > 0
                    else False
                )
            except Exception as e:
        ...
        for curr_key in self._grammar.keys():
            ...
            if isinstance(curr_value, list):
                ...
                evaluation_result = list(
                    filter(lambda x: False if self.__evaluate(x) else True, curr_value)
                )

                if not evaluation_result:
                    parsed_object[curr_key] = True
                else:
                    parsed_object[curr_key] = False
            ...
            if isinstance(curr_value, dict):
                ...
                if (
                    ("map_keys" not in curr_value) or (curr_value["map_keys"] is None)
                ) and (isinstance(raw_value, list) is False):
                    try:
                ...
                if (
                    ("map_keys" not in curr_value) or (curr_value["map_keys"] is None)
                ) and (isinstance(raw_value, list) is True):
                    try:
                ...
                if (source_array is None) or (
                    isinstance(index, numbers.Number) is False
                ):
                    continue
                ...
                computed_value = self.__value(
                    curr_value["map_path"], source_array[index]
                )
                parsed_object[curr_key] = (
                    computed_value if computed_value is not None else ""
                )
            ...
            if isinstance(curr_value, numbers.Number):
                parsed_object[curr_key] = (
                    ...
                    else float("{:.2f}".format(curr_value))
                )
